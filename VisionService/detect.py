import time
import threading

import cv2
import pafy
import tensorflow as tf

from models import yolo
from utils.general import format_predictions, find_class_by_name, is_url

FLAGS = tf.flags.FLAGS

class Detector(threading.Thread):
  def __init__(self, threadID, name, videoThread):
    threading.Thread.__init__(self)
    self.name = name
    self.winName = self.name + ' Window'
    self.currentTick = 0
    self.vidThread = videoThread

    cv2.namedWindow(self.winName)

  def run(self):
    try:
      while True:
        frame = self.vidThread.getFrame()
        self.currentTick = self.currentTick + 1
        if hasattr(frame, "__len__"):
          print(self.winName + ' frame at tick:' + str(self.currentTick))
          cv2.imshow(self.winName, frame)  
          key = cv2.waitKey(1) & 0xFF
          # Exit
          if key == ord('q'):
              break

          # Take screenshot
          if key == ord('s'):
              cv2.imwrite('frame_{}.jpg'.format(time.time()), frame)
        else:
          print('no frame at:' + str(self.currentTick))




      
    finally:
      cv2.destroyAllWindows()



class VideoThread(threading.Thread):
  def __init__(self, threadID, name):
    threading.Thread.__init__(self)
    print ("Starting " + name)
    self.threadID = threadID
    self.name = name
    self.rawFrame = None
    self.frameWidth = 640
    self.frameHeight = 480

  def run(self):
    print ("Running " + self.name)

    # get video url from command line params
    video = FLAGS.video

    # setup video stream
    if is_url(video):
      videoPafy = pafy.new(video)
      video = videoPafy.getbest(preftype="mp4").url

    # capture video stream
    cam = cv2.VideoCapture(video)

    # throw error if video stream did not open
    if not cam.isOpened():
      raise IOError('Can\'t open "{}"'.format(FLAGS.video))

    # get width/height of the video stream
    self.frameHeight = cam.get(cv2.CAP_PROP_FRAME_HEIGHT)
    self.frameWidth = cam.get(cv2.CAP_PROP_FRAME_WIDTH)

    print(self.frameHeight)
    print(self.frameWidth)

    try:
      while True:
        # try to read current frame from video feed
        ret, self.rawFrame = cam.read()
        if not ret:
          print('Can\'t read video data. Potential end of stream')
          # self.exit()
          return  
        else:
          print('frame read')

    finally:
        cam.release()

    # frame_num += 1
  def getFrame(self):
    return self.rawFrame
  


# class DetectionThread(threading.Thread):
#   def __init__(self, threadID, name):
#     threading.Thread.__init__(self)
#     self.threadID = threadID
#     self.name = name
#     print ("Starting " + self.name)
#     # start_video_window()
#   def run(self):
#     print ("Running " + self.name)
#     while True:
#       self.do_detection()

#   def start_detection(self):
#     self.FLAGS = tf.flags.FLAGS
#     self.model_cls = find_class_by_name(self.FLAGS.model_name, [yolo])
#     self.model = model_cls(input_shape=(source_h, source_w, 3))
#     self.model.init()


#   def do_detection(self):
#     predictions = self.model.evaluate(frame)

#     for o in predictions:
#         x1 = o['box']['left']
#         x2 = o['box']['right']

#         y1 = o['box']['top']
#         y2 = o['box']['bottom']

#         color = o['color']
#         class_name = o['class_name']

#         # Draw box
#         cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)

#         # Draw label
#         (test_width, text_height), baseline = cv2.getTextSize(
#             class_name, cv2.FONT_HERSHEY_SIMPLEX, 0.75, 1)
#         cv2.rectangle(frame, (x1, y1),
#                       (x1+test_width, y1-text_height-baseline),
#                       color, thickness=cv2.FILLED)
#         cv2.putText(frame, class_name, (x1, y1-baseline),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 1)

#     end_time = time.time()
#     fps = fps * 0.9 + 1/(end_time - start_time) * 0.1
#     start_time = end_time

#     # Draw additional info
#     frame_info = 'Frame: {0}, FPS: {1:.2f}'.format(frame_num, fps)
#     cv2.putText(frame, frame_info, (10, frame.shape[0]-10),
#                 cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
#     # logger.info(frame_info)

#     cv2.imshow(win_name, frame)

def run_detection(_):
  # Create new threads
  vidThread = VideoThread(1, "vidThread")
  dector1 = Detector(2, 'cam1', vidThread)
  vidThread.start()
  dector1.start()
  dector1.join()
  vidThread.join()



  print ("Exiting Main Thread")

if __name__ == '__main__':
  tf.flags.DEFINE_string('video', 0, 'Path to the video file.')
  tf.flags.DEFINE_string('model_name', 'Yolo2Model', 'Model name to use.')

  tf.app.run(main=run_detection)
