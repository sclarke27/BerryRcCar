...
class BotActions {
    ...
    handlePingSensors(sensors) {
        ...
        if (sensorData.leftDistance.current < minSensorDist && !currState.obstacles.left) {
            this._botState.setStateValue('obstacles', 'left', true);
        }
        if (sensorData.leftDistance.current > minSensorDist && currState.obstacles.left) {
            this._botState.setStateValue('obstacles', 'left', false);
        }
        if (sensorData.centerDistance.current < minSensorDist && !currState.obstacles.center) {
            this._botState.setStateValue('obstacles', 'center', true);
        }
        if (sensorData.centerDistance.current > minSensorDist && currState.obstacles.center) {
            this._botState.setStateValue('obstacles', 'center', false);
        }
        if (sensorData.rightDistance.current < minSensorDist && !currState.obstacles.right) {
            this._botState.setStateValue('obstacles', 'right', true);
        }
        if (sensorData.rightDistance.current > minSensorDist && currState.obstacles.right) {
            this._botState.setStateValue('obstacles', 'right', false);
        }
    }
    ...
    handleDriveForward(sensors, botActions) {
        const sensorData = sensors.getSensorDataSet();
        const currState = this._botState.getFullState();
